...
type Template struct {       
	...
	uT1: Template          // assim como declaramos os modulos utilizados por este modulo
	                       // os canais oferecidos por este estao na sua definicao
	state: int			   // teriamos ainda aqui tudo referente ao estado deste modulo
}

func NewTemplate( x Template // used template deveria vir com referencia aos modulos que vai utilizar	
	             ) *Template {
    ...
    TemplateEvents () {    // aqui escrevemos o tratamento de eventos - funcao dentro de NewTemplate
	   select {
		   case <-my.req:
			 
		   case <-uT1.ind:  // aqui pode mandar msg para my.ind:     my.ind <- msgind
	...
	go func() { for { templateEvents() }  } // codigo fixo indicando que fica para sempre tratando eventos	
